...
export async function getTopGainersBitquery(
    ...
    if (chain === 1 || chain === 56) {
        ...
        const payload = JSON.stringify({
            query: `query ($network: EthereumNetwork!, $limit: Int, $from: ISO8601DateTime, $till: ISO8601DateTime, $quoteCurrencies: [String!], $exchangeAddresses: [String!]) {
        ...
        if (data && data?.data?.ethereum?.dexTrades) {
            // console.log(
            //   'data',
            //   data?.data?.ethereum?.dexTrades.find((t: any) => t.baseCurrency.symbol === 'CSIX'),
            // )
            ...
            return (
                filteredTrades
                    ?.map((c: any) => {
                        ...
                        const priceUSD = nativPrice.times(
                            nativeCoinPriceOfChain
                        );
                        return {
        ...
        if (data && data?.data?.EVM?.DEXTradeByTokens) {
            ...
            return (
                filteredTrades
                    ...
                    .sort(
                        (a: any, b: any) => Number(b.change) - Number(a.change)
                    ) as TopPairResponse
            );
        }
    }
