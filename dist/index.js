...
// src/templates/topvolume.ts
var topvolumeParamTemplate = `Given the recent messages and information below:
...
\`\`\`
`;
var topvolumeTemplate = `Given the recent messages and information below:
...
'
\u{1F4C8} Top Gainers (24h) on Base Chain:
...
Harness these insights for strategic foresight! \u{1F680}
'

`;

// src/bitquery/utils/client.ts
var accessToken = "";
async function getBitqueryToken(_runtime) {
  if (accessToken && accessToken !== "") return;
  const bitQueryClientId = _runtime.character.settings?.secrets?.BITQUERY_CLIENT_ID || process.env.BITQUERY_CLIENT_ID;
  const bitQueryClientSecret = _runtime.character.settings?.secrets?.BITQUERY_CLIENT_SECRET || process.env.BITQUERY_CLIENT_SECRET;
...
async function fetchBitquery(_runtime, apiVersion, payload, fetchingWhat) {
  try {
    if (!payload) return {};
    if (fetchingWhat) {
    }
    ...
    switch (apiVersion) {
      case "https://graphql.bitquery.io" /* v1 */:
        requestHeaders.set(
          "X-API-KEY",
          _runtime.character.settings?.secrets?.BITQUERY_KEY || process.env.BITQUERY_KEY
        );
        break;
      case "https://streaming.bitquery.io/graphql" /* v2 */:
    ...
    if (data && data.errors) {
      if (fetchingWhat) {
        ...
        errors.forEach((err) => {
          console.log(
            `[Bitquery Error] Function: ${fetchingWhat} Message: ${err.message} location: ${JSON.stringify(err.locations)}`
          );
        });
      }
    }
...
// src/bitquery/config/chainmap.ts
var chainMapBitqueryV2 = /* @__PURE__ */ new Map([
  ["1", "eth"],
  ["56", "bsc"],
  ["8453", "base"]
]);

// src/bitquery/config/chainaddresses.ts
var NativeCoinOfChain = /* @__PURE__ */ new Map([
  [
    1,
    {
      symbol: "ETH",
...
var WrappedNativeCoinOfChain = /* @__PURE__ */ new Map([
  [
    1,
    {
      symbol: "WETH",
...
var StableCoinsOfChain = /* @__PURE__ */ new Map([
  [
    1,
    [
      {
...
var ChainQuoteCurrencies = /* @__PURE__ */ new Map([
  [
    1,
    [
...
var ChainDexProtocols = /* @__PURE__ */ new Map([
  [
    1,
    [
      {
...
// src/bitquery/totalsupply.ts
import { createPublicClient, formatEther, http } from "viem";
import { mainnet, base, bsc } from "viem/chains";
import { createRequire } from "module";
var require2 = createRequire(import.meta.url);
var ERC20 = require2("@openzeppelin/contracts/build/contracts/ERC20.json");
async function getWeb3TotalSupply(runtime, chain, address) {
  const infura_api_key = process.env.INFURA_API_KEY || runtime?.character?.settings?.secrets?.["INFURA_API_KEY"];
  if (!infura_api_key || infura_api_key === "")
    throw new Error(
      "INFURA_API_KEY is not set in secrets or env variables"
    );
  const transportUrl = chain === 1 ? "https://mainnet.infura.io/v3/" : chain === 56 ? "" : chain === 8453 ? "https://base-mainnet.infura.io/v3/" : null;
  const viewmChain = chain === 1 ? mainnet : chain === 56 ? bsc : chain === 8354 ? base : null;
  try {
...
// src/bitquery/topvolume.ts
import BigNumber from "bignumber.js";
function calcPercentage(startprice, endprice) {
...
async function getTopPairsByVolBitquery(runtime, chain, limit, timeperiod, minSwaps = 400) {
  let timeframe = "hour";
  let interval = 24;
  let from = new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString();
  if (timeperiod) {
    switch (timeperiod.toString().toLowerCase()) {
      case "5m":
        timeframe = "minute";
        interval = 5;
        from = new Date(Date.now() - 5 * 60 * 1e3).toISOString();
        break;
      case "1h":
        timeframe = "hour";
        interval = 1;
        from = new Date(Date.now() - 60 * 60 * 1e3).toISOString();
        break;
      case "3h":
        timeframe = "hour";
        interval = 3;
        from = new Date(Date.now() - 3 * 60 * 60 * 1e3).toISOString();
        break;
      case "12h":
        timeframe = "hour";
        interval = 12;
        from = new Date(Date.now() - 12 * 60 * 60 * 1e3).toISOString();
        break;
      case "24h":
        timeframe = "hour";
        from = new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString();
        break;
      case "7d":
        timeframe = "day";
        interval = 7;
  ...
  const data = await fetchBitquery(runtime, "https://streaming.bitquery.io/graphql" /* v2 */, payload);
  if (data && data?.data?.EVM?.DEXTradeByTokens) {
    ...
    const result = await Promise.all(
      filteredTrades?.map(async (c) => {
        let totalSupply = null;
        if (process.env.INFURA_API_KEY || runtime?.character?.settings?.secrets?.["INFURA_API_KEY"]) {
          try {
        ...
        return {
          ...
          change: calcPercentage(c?.Trade?.open, c?.Trade?.close),
          // cutToDigits(c?.final, 2),
          priceType: `${calcPercentage(c?.Trade?.open, c?.Trade?.close)}`.substring(
            0,
            1
          ) === "-" ? "-" : "+",
          // c?.final
          volume: c?.volume,
          nativePrice: null,
          //  +c?.close_price, // native price
          price: +c?.Trade?.close,
          // price in USD
          marketCap: totalSupply ? new BigNumber(c?.Trade?.close).times(new BigNumber(totalSupply)).toFixed(0) : null,
          totalSupply: totalSupply ? new BigNumber(totalSupply).toFixed(0) : null,
          swaps: c?.count,
          quoteCurrency: {
...
// src/environment.ts
var bitqueryEnvSchema = z.object({
...
async function validateBitqueryConfig(runtime) {
  try {
    const config = {
...
// src/actions/topvolume.ts
var cache = {};
var fetchTopTokens = async (runtime, chain, timeperiod, limit) => {
  const now = Date.now();
  const cacheEntry = cache[chain];
  const CACHE_DURATION_MINUTES = Number(runtime.character?.settings?.secrets?.CACHE_DURATION_MINUTES) || 0;
  if (cacheEntry && now - cacheEntry.timestamp < CACHE_DURATION_MINUTES * 60 * 1e3) {
...
var formatMarketData = (chain, timeperiod, topVolume) => {
  const chainLabel = chain === 8453 /* Base */ ? "Base" : chain === 56 /* BinanceSmartChain */ ? "Binance Smart Chain (BSC)" : "Ethereum";
  const fetchedAt = (/* @__PURE__ */ new Date()).toLocaleString();
...
var formatSection = (sectionTitle, pairs, timeperiod) => {
...
`;
  if (pairs.length < 100) {
    formattedSection += pairs.map(
      (pair) => `Token name: ${pair.name}, ticker: ${pair.symbol}, address: ${pair.address}, ${timeperiod} price change: ${pair.change}%, ${timeperiod} volume: ${pair.volume}$, swaps: ${pair?.swaps}, ${pair?.nativePrice ? `nativePrice (in native coin of chain): ${pair.nativePrice},` : ""} ${pair?.price ? `price (in usd): ${pair.price}$,` : ""} ${pair?.marketCap && Number(pair?.marketCap) !== 0 ? `marketCap: ${pair?.marketCap},` : ""} ${pair?.totalSupply && Number(pair?.totalSupply) !== 0 ? `totalsupply: ${pair.totalSupply},` : ""} `
    ).join("\n");
  } else {
    formattedSection += pairs.map(
      (pair) => `Token name: ${pair.name}, ticker: ${pair.symbol}, ${timeperiod} price change: ${pair.change}%, ${timeperiod} volume: ${pair.volume}$, swaps: ${pair?.swaps}, ${pair?.nativePrice ? `nativePrice (in native coin of chain): ${pair.nativePrice},` : ""} ${pair?.price ? `price (in usd): ${pair.price}$,` : ""} ${pair?.marketCap && Number(pair?.marketCap) !== 0 ? `marketCap: ${pair?.marketCap},` : ""} ${pair?.totalSupply && Number(pair?.totalSupply) !== 0 ? `totalsupply: ${pair.totalSupply},` : ""} `
    ).join("\n");
  }
  formattedSection += "\n";
  return formattedSection;
};
var getTopTokensByVolAction = {
  name: "TOP_TOKENS",
  description: "query the top tokens by volume on base, eth, arbitrum or bsc network",
  ...
  handler: async (_runtime, message, state, _options, callback) => {
    ...
    const timeperiods = [params.timeperiod];
...
// src/bitquery/pairs.ts
import BigNumber2 from "bignumber.js";
function calcPercentage2(startprice, endprice) {
  const percent = (endprice - startprice) / startprice * 100;
  if (!isNaN(percent)) {
    return percent.toFixed(2);
  } else {
...
async function getPairsBitquery(runtime, chain, baseCurrencies, limit, timeperiod, minSwaps = 400) {
  let timeframe = "hour";
  let interval = 24;
  let from = new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString();
  if (timeperiod) {
    ...
    const result = await Promise.all(
      filteredTrades?.map(async (c) => {
        ...
        return {
          ...
          decimals: c?.Trade?.Currency?.Decimals,
          name: c?.Trade?.Currency?.Name,
          change: calcPercentage2(c?.Trade?.open, c?.Trade?.close),
          // cutToDigits(c?.final, 2),
          priceType: `${calcPercentage2(c?.Trade?.open, c?.Trade?.close)}`.substring(
            0,
            1
          ) === "-" ? "-" : "+",
          // c?.final
          ...
          price: +c?.Trade?.close,
          // price in USD
          marketCap: totalSupply ? new BigNumber2(c?.Trade?.close).times(new BigNumber2(totalSupply)).toFixed(0) : null,
          totalSupply: totalSupply ? new BigNumber2(totalSupply).toFixed(0) : null,
          swaps: c?.count,
          quoteCurrency: {
            symbol: c?.Trade?.Side?.Currency?.Symbol,
            address: c?.Trade?.Side?.Currency?.SmartContract
...
// src/bitquery/latestpairs.ts
async function getLatestPairsBitqueryNew(runtime, chain, timeperiod, limit) {
  ...
  const data = await fetchBitquery(
    runtime,
    "https://streaming.bitquery.io/graphql" /* v2 */,
    payload,
  ...
  const addressesWithDates = /* @__PURE__ */ new Map();
  data?.data.EVM.Events.forEach((event) => {
    const blockTime = event.Block.Time;
    let includeEvent = false;
    const tokenAddresses = [];
    event.Arguments.forEach((arg) => {
      if ((arg.Name === "token0" || arg.Name === "token1") && arg.Value.__typename === "EVM_ABI_Address_Value_Arg") {
  ...
  const latestTokens = Array.from(addressesWithDates.keys()).filter(
    (address) => address.toLowerCase() !== NativeCoinOfChain.get(chain).address.toLowerCase() && address.toLowerCase() !== StableCoinsOfChain.get(chain)?.[0]?.address.toLowerCase()
  );
  ...
  const enhancedPairs = pairs.map((pair) => {
    return {
      ...pair,
      created: addressesWithDates.get(pair.address) || null
...
// src/templates/latestpairs.ts
var latestPairsParamTemplate = `Given the recent messages and information below:
...
\`\`\`
`;
var latestPairsTemplate = `Given the recent messages and information below:
...
1. Clank Tank (TANK) \u{1F680} +484.13%, 2525 swaps
2. GnomeLand (GNOMELAND) \u{1F31F} +124.39%, 9255 swaps

Exciting moves, stay curious but cautious! \u{1F4C8}
'''
...
// src/actions/latestpairs.ts
var cache2 = {};
var fetchLatestPairs = async (runtime, chain, timeperiod, limit) => {
  const now = Date.now();
  const cacheEntry = cache2[chain];
  const CACHE_DURATION_MINUTES = Number(runtime.character?.settings?.secrets?.CACHE_DURATION_MINUTES) || 0;
  if (cacheEntry && now - cacheEntry.timestamp < CACHE_DURATION_MINUTES * 60 * 1e3) {
    console.log(`Returning cached data for chain ${chain}`);
    return cacheEntry.data;
  ...
  try {
    ...
    cache2[chain] = { data: latestPairs, timestamp: now };
    return latestPairs;
  } catch (error) {
    console.error(
      `Error fetching top volume tokens data for chain ${chain}:`,
...
var formatMarketData2 = (chain, timeperiod, latestPairs) => {
  const chainLabel = chain === 8453 /* Base */ ? "Base" : chain === 56 /* BinanceSmartChain */ ? "Binance Smart Chain (BSC)" : "Ethereum";
  const fetchedAt = (/* @__PURE__ */ new Date()).toLocaleString();
  let marketKnowledge = `Latest deployed Tokens by volume on ${chainLabel} in the past ${timeperiod} (fetched on ${fetchedAt}):
`;
  marketKnowledge += formatSection2("Latest Tokens", latestPairs, timeperiod);
  return marketKnowledge;
};
var formatSection2 = (sectionTitle, pairs, timeperiod) => {
  if (!pairs || pairs.length === 0) {
    return `${sectionTitle}: No data available.
`;
  }
...
var getLatestPairsAction = {
  ...
  handler: async (_runtime, message, state, _options, callback) => {
    ...
    const queryContext = composeContext2({
      state,
      template: latestPairsParamTemplate
    });
    const params = await generateObjectDeprecated2({
      runtime: _runtime,
      context: queryContext,
      ...
      if (chain !== null) {
        for (const timeperiod of timeperiods) {
          ...
          try {
            ...
            const marketData = formatMarketData2(
              chain ?? 8453,
              timeperiod ?? "24h",
              latestPairs
            );
    ...
    if (onchainKnowledge !== "") {
      ...
      const convictionResponse = await generateText2({
        runtime: _runtime,
        context,
        modelClass: ModelClass2.LARGE
      });
...
// src/bitquery/chart.ts
async function getTokenChart(runtime, chain, baseCurrency, timeperiod, quoteCurrency) {
  const chainid = chain;
  ...
  const wrappedNativeCoin = WrappedNativeCoinOfChain.get(Number(chain))?.symbol || null;
  ...
  const isNativeOrWrapped = baseCurrency?.toLowerCase() === nativeCoin?.toLowerCase() || baseCurrency?.toLowerCase() === wrappedNativeCoin?.toLowerCase() || baseCurrency?.toLowerCase() === WrappedNativeCoinOfChain.get(chain)?.address?.toLowerCase() || baseCurrency?.toLowerCase() === NativeCoinOfChain.get(chain)?.address?.toLowerCase() || baseCurrency?.toLowerCase() === NativeCoinOfChain.get(chain)?.name?.toLowerCase() || baseCurrency?.toLowerCase() === WrappedNativeCoinOfChain.get(chain)?.name?.toLowerCase();
  quotecurrency = isNativeOrWrapped ? StableCoinsOfChain.get(Number(chain))?.[0]?.address || null : ChainQuoteCurrencies.get(Number(chain))?.[0] || null;
  if (!quotecurrency) {
  ...
  const fromISO = orderBy === "Block_Time" ? from.toISOString() : from.toISOString().split("T")[0];
  const till = orderBy === "Block_Time" ? (/* @__PURE__ */ new Date()).toISOString() : (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  ...
  if (!quoteCurrency) {
    ...
    quoteCurrency = quoteCurrency.charAt(0).toUpperCase() + quoteCurrency.slice(1).toLowerCase();
  }
    ...
    query ($network: evm_network, $baseCurrency: String, $quoteCurrency: String, $from: ${blockTimeType}, $till: ${blockTimeType}, $exchangeAddresses: [String!]) {
        EVM(dataset: combined, network: $network) {
            DEXTradeByTokens(
                ...
                Block {
                    ${blockTimeField === "Time" ? `Time(interval: {in: ${interval}, count: ${intervalCount}})` : "Date"}
                }
...
// src/templates/chart.ts
var tokenChartParamTemplate = `Given the recent messages and information below:
...
\`\`\`
`;
var tokenChartTemplate = `Given the recent messages and information below:
...
'''
\u{1F4CA} VIRTUAL 7-Day Analysis:
...
// src/actions/chart.ts
var cache3 = {};
var fetchTokenChart = async (runtime, chain, baseCurrency, timeperiod) => {
  const cacheKey = `${chain}-${baseCurrency}-${timeperiod}`;
  const now = Date.now();
  const cacheEntry = cache3[cacheKey];
  const CACHE_DURATION_MINUTES = Number(runtime.character?.settings?.secrets?.CACHE_DURATION_MINUTES) || 0;
  if (cacheEntry && now - cacheEntry.timestamp < CACHE_DURATION_MINUTES * 60 * 1e3) {
  ...
  try {
    ...
    cache3[cacheKey] = { data: tokenChartData, timestamp: now };
...
var formatTokenChartData = (chain, baseCurrency, chartData) => {
  const chainLabel = chain === 1 /* Ethereum */ ? "Ethereum" : chain === 56 /* BinanceSmartChain */ ? "Binance Smart Chain (BSC)" : "Base";
  let formattedData = `Historical price chart for ${baseCurrency} on ${chainLabel}:
`;
  formattedData += chartData.map((price) => `Time: ${price.time}, Price: $${price.close}`).join("\n");
  return formattedData;
};
var getTokenChartAction = {
  name: "TOKEN_CHART",
  description: "Query historical token prices to analyse the chart of a token on a specific blockchain",
  suppressInitialMessage: true,
  handler: async (_runtime, message, state, _options, callback) => {
    ...
    const queryContext = composeContext3({
      state,
      template: tokenChartParamTemplate
    });
    const params = await generateObjectDeprecated3({
      runtime: _runtime,
      context: queryContext,
    ...
    try {
      ...
      const processedResponse = await generateText3({
        runtime: _runtime,
        context,
        modelClass: ModelClass3.LARGE
      });
...
var bitqueryPlugin = {
  ...
  // onchainProvider, factsProvider, toptokenProvider
  evaluators: [],
  services: [],
  actions: [
...
var index_default = bitqueryPlugin;
export {
  bitqueryPlugin,
